...
namespace AccountManager.ViewModels
{
    ...
    public class AccountDialogViewModel : BaseDialogViewModel
    {
        ...
        public bool CanSave => IsValidAccount();
        ...
        {
            if (string.IsNullOrWhiteSpace(Account?.Name) && string.IsNullOrWhiteSpace(Account?.Password))
            {
                _notificationService.ShowWarning("Please provide both an account name and password to continue.", "Missing Required Fields");
                return false;
            }
            else if (string.IsNullOrWhiteSpace(Account?.Name))
            {
                _notificationService.ShowWarning("Please provide an account name to continue.", "Account Name Required");
                return false;
            }
            else if (string.IsNullOrWhiteSpace(Account?.Password))
            {
                _notificationService.ShowWarning("Please provide a password for this account.", "Password Required");
                return false;
            }
            return true;
        }
        ...
        {
            if (e.PropertyName == nameof(Account.Name) || e.PropertyName == nameof(Account.Password))
            {
                OnPropertyChanged(nameof(CanSave));
            }
        }
